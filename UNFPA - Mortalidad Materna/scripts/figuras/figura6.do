...
foreach group in `groups' {
	...
	restore, preserve
	collapse usa_anticonceptivo if `group' == 1 & edad_reproductiva == 1 & en_union == 1 [iweight = V005], by(year)
	ren usa_anticonceptivo `group'
...
export delimited using "intermediate/figura6.csv", delimiter(",") replace